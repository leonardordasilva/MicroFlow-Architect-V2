...
serve(async (req) => {
  ...
  try {
    ...
    // Official Supabase edge function pattern: anon key + user's JWT via global header
    ...
    console.log("[stripe-checkout] authHeader prefix:", authHeader?.substring(0, 20));
    const { data: { user }, error: authError } = await supabase.auth.getUser();
    console.log("[stripe-checkout] getUser:", { userId: user?.id, error: authError?.message });
    ...
    const appUrl = clientAppUrl || Deno.env.get("APP_URL") || "https://localhost:5173";
